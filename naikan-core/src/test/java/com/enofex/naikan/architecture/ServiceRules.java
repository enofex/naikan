...
final class ServiceRules {
  ...
  static List<DynamicTest> serviceRules(ArchUnitTestsConfig config) {
    return List.of(
        dynamicTest("Service implementations should be named *ServiceHandler",
            () -> classes()
                .that().implement(nameMatching(SERVICE))
                .should().haveNameMatching(".*ServiceHandler")
                .check(config.getClasses())),

        dynamicTest("Service implementations should reside in support package",
            () -> classes()
                .that().implement(nameMatching(SERVICE))
